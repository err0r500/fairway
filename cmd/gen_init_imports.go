...
func main() {
	// Configuration
	scanDirs := []string{"change", "view", "automate"} // default
	if len(os.Args) > 1 {
		scanDirs = strings.Split(os.Args[1], ",")
	}
	...
	for _, scanDir := range scanDirs {
		...
		if err != nil {
			log.Fatalf("Error reading directory %s: %v", scanDir, err)
		}
		...
		for _, entry := range entries {
			if entry.IsDir() {
				...
				if hasInitFunction(subDir) {
					// Create import path
					importPath := fmt.Sprintf("%s/%s/%s", moduleName, scanDir, entry.Name())
